...
public class TourMapFragment extends MapFragment implements
		MapLoaderCallbacks.MapDataLoader, GoogleMap.OnInfoWindowClickListener, LocationListener {
		...
		if (mMap == null) {
			...
			if (mMap != null) {
				...
				if (mSavedInstanceState == null) {
					...
					mMap.moveCamera(CameraUpdateFactory.newLatLngZoom(LONDON,
							12));
				}
				...
				lm.initLoader(MapLoaderCallbacks.TOKEN_POI, null,
						mLoaderCallbacks);
				lm.initLoader(MapLoaderCallbacks.TOKEN_ROUTE, null,
						mLoaderCallbacks);

				// Set a custom info window adapter.
				TourInfoWindowAdapter adapter = new TourInfoWindowAdapter(
						getActivity(), mPoiData);
...
//		startActivity(intent);
	 ...
	 * 
	 * @param type
	 *            One of the constants listed in {@link SelectMapTypeDialog}.
	 */
	...
	public void addPoi(PointOfInterest poi) {
		MarkerOptions options = new MarkerOptions().position(poi.mLocation)
				.title(poi.mTitle).snippet(poi.mDescription);
